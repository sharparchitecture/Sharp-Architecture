...
A CodeGraph index (`.codegraph/`) is present — prefer structural exploration of the codebase
(searching symbols, call graphs, where a symbol is defined, blast radius) for structural questions
over broad grep/read loops.

## Auto-approve / safe commands

The following read-only commands are pre-approved as safe to run without confirmation
(migrated from the previous Claude `settings.local.json` permissions):
